...
return false;
/*
 * PortletCollection cs =
 * ctx.getPublication().getWorkspace().getCollaborativeSpace(); Category cat =
 * ctx.getCategory(); Member mbr = ctx.getMember();
 * 
 * // Search in topic root if (cs == null) { if
 * (cat.hasAncestor(mgr.getTopicRoot())) { ctx.setAttribute(URL_ATTR,
 * "plugins/JBookPlugin/jsp/app/jbook.jsp?cids=" + cat.getId()); return true; }
 * return false; } else { PortletJBook portlet =
 * cs.findFirstPortlet(PortletJBook.class); if (portlet != null) { Set<Category>
 * topicRootSet = portlet.getTopicRoot(mbr); if (topicRootSet != null) {
 * for(Category root: topicRootSet) { if (cat.hasAncestor(root)) {
 * ctx.setAttribute(URL_ATTR, cs.getDisplayUrl(mbr.getLocale()) + "?portlet=" +
 * portlet.getId() + "&cids=" + cat.getId()); return true; } } } else { if
 * (cat.hasAncestor(mgr.getTopicRoot())) { ctx.setAttribute(URL_ATTR,
 * cs.getDisplayUrl(mbr.getLocale()) + "?portlet=" + portlet.getId() + "&cids="
 * + cat.getId()); return true; } } } }
 * 
 * return false;
 */
  }
