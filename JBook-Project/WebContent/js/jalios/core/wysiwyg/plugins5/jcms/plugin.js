!function($) {
  tinymce.PluginManager.add('jcms', function(editor, url) {
    ...
    editor.on('PreInit', function() {
      ...
      editorBody.attr("lang", $textarea.attr("lang"));
      editorBody.attr("xml:lang", $textarea.attr("xml:lang"));
      
      // Initialize the context
      var context = $textarea.data();
      editor.jcms.context = $.extend({}, context, {
        getWorkspace : function() {
          ...
          var $editPubForm = $textarea.closest('.edit-pub');
    ...
    // --------------------------------------------------------------------------------
    // Image context menu workaround
    // --------------------------------------------------------------------------------

    // Provide custom "quickimage" context menu entry to workaround behavior change of "image" entry in TinyMCE 5
    // cf https://github.com/tinymce/tinymce/issues/6628
    ...
    editor.ui.registry.addToggleButton('spellchecker', {
      icon: 'spell-check',
      tooltip: 'Spellcheck',
      disabled: true,
      onAction: function () {
        editor.ui.registry.getAll().buttons.onAction();
      },
      onSetup: function (buttonApi) {
        var unbindHandler = editor.ui.registry.getAll().buttons.spellchecker.onSetup();
        
        var disableSpellCheckerButton = function () {
          buttonApi.setDisabled(true);
        };
        var enableSpellCheckerButton = function () {
          buttonApi.setDisabled(false);
        };
        editor.on('PostRender', disableSpellCheckerButton);
        editor.on('Focus', enableSpellCheckerButton);
        editor.on('Blur', disableSpellCheckerButton);
        
        /* onSetup should always return the unbind handlers */
        return function (buttonApi) {
          editor.off('PostRender', disableSpellCheckerButton);
          editor.off('Focus', enableSpellCheckerButton);
          editor.off('Blur', disableSpellCheckerButton);
          unbindHandler();
        };
      }
    });
    ...
    // --------------------------------------------------------------------------------
    // Spellchecker - when using it, override contextmenu with spellchecker entry only
    // --------------------------------------------------------------------------------
    
    var contextMenuBackup = "";
    editor.on('SpellcheckStart', function () {
      contextMenuBackup = editor.settings.contextmenu; 
      editor.settings.contextmenu =  "spellchecker";
    });
    editor.on('SpellcheckEnd', function () {
      editor.settings.contextmenu =  contextMenuBackup;
    });
