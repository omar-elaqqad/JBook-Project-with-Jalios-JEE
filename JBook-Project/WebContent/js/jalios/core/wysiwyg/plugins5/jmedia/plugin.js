  ...
  tinymce.PluginManager.add('jmedia', function(editor, url) {
    ...
    editor.jalios.plugins.jmedia = {
      insertTag: function(options) {
        ...
        var content = editor.jalios.plugins.jmedia.createTag(options);
    ...
    // Add a button
    editor.ui.registry.addIcon('jmedia', '<span class="jalios-icon icomoon-film4"></span>' );
    editor.ui.registry.addToggleButton('jmedia', {
      ...
      onSetup : function(buttonApi) {
        const nodeChangeHandler = function (eventApi) {
          buttonApi.setActive(isMedia(eventApi.element));
        };
    ...
    // Adds a menu item
    /*
     * editor.ui.registry.addMenuItem('publication', { text: 'JMedia plugin', context: 'tools', onAction: openChooser });
     */
