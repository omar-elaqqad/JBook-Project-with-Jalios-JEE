  ...
  tinymce.PluginManager.add('jlink', function(editor, url) {
    ...
    editor.jalios.plugins.jlink = {
        ...
        createHtmlTag: function(options) {
          var attr = ' href="' + options.url + '" ' + htmlDataAttr + '="' + $.wysiwyg.utils.escape(options.source) + '"';
    ...
    // Add a button
    editor.addButton('jlink', {
      tooltip : tinymce.i18n.translate('jlink.btn.tooltip'),
      icon : 'icomoon icomoon-file-text2',
      onclick : openChooser,

      onpostrender : function() {
        var btn = this;
        editor.on('NodeChange', function(e) {
          btn.active(isLink(e.element));
        });
      }
    });
    ...
    // Adds a menu item
    /*
     * editor.addMenuItem('jlink', { text: 'JLink plugin', context: 'tools', onclick: openChooser });
     */
    ...
    function restoreContent(content) {
      content = content.replace(placeholderRegExp, function($0) {
        ...
        if ($elt.attr('href') === cachedMediaInfos.url) {
          var mediaInfos = {
            source : source,
