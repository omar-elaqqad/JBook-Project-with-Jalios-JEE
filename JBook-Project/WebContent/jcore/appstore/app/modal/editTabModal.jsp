...
<jalios:modal url="jcore/appstore/app/modal/editTabModal.jsp" title='<%= glp("application-launcher.edit-tab") %>'>
	...
	<jalios:buffer name="MODAL_BUTTON">
		<button type="button" class="btn btn-primary btn-tab-rename-validate"><%= glp("ui.com.btn.update") %></button>
	</jalios:buffer>
    ...
    <%  String label = JcmsUtil.glp(lang, "lang") + flag; %>
	  ...
	  <jalios:field name="<%= lang %>" label="<%= label %>" value="<%= formHandler.getTabName(lang) %>" required="<%= formHandler.isDefaultLang(lang) %>">
