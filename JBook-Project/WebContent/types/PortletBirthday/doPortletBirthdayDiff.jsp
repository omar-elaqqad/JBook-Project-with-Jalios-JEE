...
<%--
  @Summary: generated.PortletBirthday diff page
  @Category: Generated
...
<% 
{ 
generated.PortletBirthday newObj = (generated.PortletBirthday)request.getAttribute("newPub"); 
generated.PortletBirthday oldObj = (generated.PortletBirthday)request.getAttribute("oldPub"); 
String[] vids = (String[])request.getAttribute("vids"); 
int fieldCount = 1; 
TypeEntry typeEntry = channel.getTypeEntry(generated.PortletBirthday.class);
...
<% } %>
  ...
  <%-- ** workspaceMembers ***************** --%>
  <jalios:if predicate='<%= newObj.getWorkspaceMembers() != oldObj.getWorkspaceMembers() %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.PortletBirthday.class, "workspaceMembers", userLang) %></td>
    <td><%= newObj.getWorkspaceMembersLabel(userLang) %></td>
    <td><%= oldObj.getWorkspaceMembersLabel(userLang) %></td>
  </tr>
  </jalios:if>
  <%-- ** relationshipFilter ***************** --%>
  <jalios:if predicate='<%= newObj.getRelationshipFilter() != oldObj.getRelationshipFilter() %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.PortletBirthday.class, "relationshipFilter", userLang) %></td>
    <td><%= newObj.getRelationshipFilterLabel(userLang) %></td>
    <td><%= oldObj.getRelationshipFilterLabel(userLang) %></td>
  </tr>
  </jalios:if>
<%
