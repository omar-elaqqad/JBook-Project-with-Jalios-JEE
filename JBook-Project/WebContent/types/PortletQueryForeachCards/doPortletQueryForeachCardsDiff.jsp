...
<%--
  @Summary: generated.PortletQueryForeachCards diff page
  @Category: Generated
...
<% 
{ 
generated.PortletQueryForeachCards newObj = (generated.PortletQueryForeachCards)request.getAttribute("newPub"); 
generated.PortletQueryForeachCards oldObj = (generated.PortletQueryForeachCards)request.getAttribute("oldPub"); 
String[] vids = (String[])request.getAttribute("vids"); 
int fieldCount = 1; 
TypeEntry typeEntry = channel.getTypeEntry(generated.PortletQueryForeachCards.class);
...
<% } %>
  ...
  <%-- ** cardsPerLine ***************** --%>
  <jalios:if predicate='<%= !Util.isSameContent(newObj.getCardsPerLine(), oldObj.getCardsPerLine()) %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.PortletQueryForeachCards.class, "cardsPerLine", userLang) %>
    </td>
    <td><%= Util.getString(newObj.getCardsPerLine(), "") %></td>
    <td><%= Util.getString(oldObj.getCardsPerLine(), "") %></td>
  </tr>
  </jalios:if>
  <%-- ** cardCssClass ***************** --%>
  <jalios:if predicate='<%= !Util.isSameContent(newObj.getCardCssClass(), oldObj.getCardCssClass()) %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <% fieldDomId = "cardCssClassDiffDomId"; %>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.PortletQueryForeachCards.class, "cardCssClass", userLang) %>
    </td>
    <% 
      request.setAttribute("fieldDomId", fieldDomId);
      request.setAttribute("step", "action");
    ...
    <%
    {
    String newCardCssClass = newObj.getCardCssClass() == null ? "" : Util.toWordLine(newObj.getCardCssClass());
    String oldCardCssClass = oldObj.getCardCssClass() == null ? "" : Util.toWordLine(oldObj.getCardCssClass());
    request.setAttribute("newField", newCardCssClass);
    request.setAttribute("oldField", oldCardCssClass);
    request.setAttribute("step", "show_fields");
    %>
    <jsp:include page="/work/doVersionToggleMergeFields.jsp" />
        <div class="merge_fields" >
    <%
    BufferedReader newReader = new BufferedReader(new StringReader(newCardCssClass));
    BufferedReader oldReader = new BufferedReader(new StringReader(oldCardCssClass));
    Diff diff = new Diff(); 
    diff.diffBuffer(oldReader , newReader);
    newReader.close();
    oldReader.close();
    oldReader  = new BufferedReader(new StringReader(oldCardCssClass));
    %>
     <%= diff.iterateHunkBuffer(oldReader, "<span class='diff-remove'>" , "</span>" , "<span class='diff-add'>" , "</span>", "" , "", false,true,false) %>
    <% 
    oldReader.close(); 
