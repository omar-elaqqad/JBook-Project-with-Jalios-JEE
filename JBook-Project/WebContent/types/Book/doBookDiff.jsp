...
<%--
  @Summary: generated.Book diff page
  @Category: Generated
...
<% 
{ 
generated.Book newObj = (generated.Book)request.getAttribute("newPub"); 
generated.Book oldObj = (generated.Book)request.getAttribute("oldPub"); 
String[] vids = (String[])request.getAttribute("vids"); 
int fieldCount = 1; 
TypeEntry typeEntry = channel.getTypeEntry(generated.Book.class);
...
<% } %>
  ...
  <tr class="vTop">
    ...
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.Book.class, "description", userLang) %>
      <jalios:if predicate='<%= channel.isMultilingual() %>'><jalios:lang lang='<%= itLang %>'/></jalios:if>
      <% fieldDomId += "_" + itLang; %>
    </td>
    <% 
  ...
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.Book.class, "image", userLang) %>
    </td>
  ...
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <% fieldDomId = "isbnDiffDomId"; %>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.Book.class, "isbn", userLang) %>
    </td>
    <% 
      request.setAttribute("fieldDomId", fieldDomId);
      request.setAttribute("step", "action");
    ...
    <%
    {
    String newIsbn = newObj.getIsbn() == null ? "" : Util.toWordLine(newObj.getIsbn());
    String oldIsbn = oldObj.getIsbn() == null ? "" : Util.toWordLine(oldObj.getIsbn());
    request.setAttribute("newField", newIsbn);
    request.setAttribute("oldField", oldIsbn);
    request.setAttribute("step", "show_fields");
    %>
    <jsp:include page="/work/doVersionToggleMergeFields.jsp" />
        <div class="merge_fields" >
    <%
    BufferedReader newReader = new BufferedReader(new StringReader(newIsbn));
    BufferedReader oldReader = new BufferedReader(new StringReader(oldIsbn));
    Diff diff = new Diff(); 
    diff.diffBuffer(oldReader , newReader);
    newReader.close();
    oldReader.close();
    oldReader  = new BufferedReader(new StringReader(oldIsbn));
    %>
     <%= diff.iterateHunkBuffer(oldReader, "<span class='diff-remove'>" , "</span>" , "<span class='diff-add'>" , "</span>", "" , "", false,true,false) %>
    <% 
    oldReader.close(); 
