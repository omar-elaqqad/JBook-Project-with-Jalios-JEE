...
<%--
  @Summary: generated.WebPage diff page
  @Category: Generated
...
<% 
{ 
generated.WebPage newObj = (generated.WebPage)request.getAttribute("newPub"); 
generated.WebPage oldObj = (generated.WebPage)request.getAttribute("oldPub"); 
String[] vids = (String[])request.getAttribute("vids"); 
int fieldCount = 1; 
TypeEntry typeEntry = channel.getTypeEntry(generated.WebPage.class);
...
<% } %>
  ...
  <%-- ** url ***************** --%>
  <jalios:if predicate='<%= !Util.isSameContent(newObj.getUrl(), oldObj.getUrl()) %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.WebPage.class, "url", userLang) %>
    </td>
    <td><%= Util.getString(newObj.getUrl(), "") %></td>
    <td><%= Util.getString(oldObj.getUrl(), "") %></td>
  </tr>
  </jalios:if>
  <%-- ** urlToShot ***************** --%>
  <jalios:if predicate='<%= !Util.isSameContent(newObj.getUrlToShot(), oldObj.getUrlToShot()) %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.WebPage.class, "urlToShot", userLang) %>
    </td>
    <td><%= Util.getString(newObj.getUrlToShot(), "") %></td>
    <td><%= Util.getString(oldObj.getUrlToShot(), "") %></td>
  </tr>
  </jalios:if>
  <%-- ** description ***************** --%>
  <jalios:foreach collection='<%= channel.getLanguageList() %>' name='itLang' type='String'>
  ...
  <tr class="vTop">
    ...
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.WebPage.class, "description", userLang) %>
      <jalios:if predicate='<%= channel.isMultilingual() %>'><jalios:lang lang='<%= itLang %>'/></jalios:if>
      <% fieldDomId += "_" + itLang; %>
    </td>
    <% 
