...
<%--
  @Summary: generated.PortletPoll diff page
  @Category: Generated
...
<% 
{ 
generated.PortletPoll newObj = (generated.PortletPoll)request.getAttribute("newPub"); 
generated.PortletPoll oldObj = (generated.PortletPoll)request.getAttribute("oldPub"); 
String[] vids = (String[])request.getAttribute("vids"); 
int fieldCount = 1; 
TypeEntry typeEntry = channel.getTypeEntry(generated.PortletPoll.class);
...
<% } %>
  ...
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.PortletPoll.class, "hideWhenNoResults", userLang) %></td>
    <td><%= newObj.getHideWhenNoResultsLabel(userLang) %></td>
    <td><%= oldObj.getHideWhenNoResultsLabel(userLang) %></td>
  </tr>
  </jalios:if>
  <%-- ** poll ***************** --%>
  <jalios:if predicate='<%= newObj.getPoll() != oldObj.getPoll() %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.PortletPoll.class, "poll", userLang) %></td>
    <td>
      <% Data newLink = newObj.getPoll(); %>
      <% if (newLink != null) { %> 
  ...
  <%-- ** refineWorkspace ***************** --%>
  <jalios:if predicate='<%= !Util.isSameContent(newObj.getRefineWorkspace(), oldObj.getRefineWorkspace()) %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.PortletPoll.class, "refineWorkspace", userLang) %>
    </td>
    <td><%= Util.getString(newObj.getRefineWorkspace(), "") %></td>
    <td><%= Util.getString(oldObj.getRefineWorkspace(), "") %></td>
  </tr>
  </jalios:if>
<%
