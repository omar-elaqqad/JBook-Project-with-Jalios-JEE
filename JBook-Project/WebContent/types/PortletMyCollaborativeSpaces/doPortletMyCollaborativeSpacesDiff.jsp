...
<%--
  @Summary: generated.PortletMyCollaborativeSpaces diff page
  @Category: Generated
...
<% 
{ 
generated.PortletMyCollaborativeSpaces newObj = (generated.PortletMyCollaborativeSpaces)request.getAttribute("newPub"); 
generated.PortletMyCollaborativeSpaces oldObj = (generated.PortletMyCollaborativeSpaces)request.getAttribute("oldPub"); 
String[] vids = (String[])request.getAttribute("vids"); 
int fieldCount = 1; 
TypeEntry typeEntry = channel.getTypeEntry(generated.PortletMyCollaborativeSpaces.class);
...
<% } %>
  ...
  <%-- ** depth ***************** --%>
  <jalios:if predicate='<%= newObj.getDepth() != oldObj.getDepth() %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.PortletMyCollaborativeSpaces.class, "depth", userLang) %></td>
    <td><%= newObj.getDepth()  %></td>
    <td><%= oldObj.getDepth()  %></td>
  </tr>
  </jalios:if>
  <%-- ** typologyFilter ***************** --%>
  <jalios:if predicate='<%= newObj.getTypologyFilter() != oldObj.getTypologyFilter() %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.PortletMyCollaborativeSpaces.class, "typologyFilter", userLang) %></td>
    <td><%= newObj.getTypologyFilterLabel(userLang) %></td>
    <td><%= oldObj.getTypologyFilterLabel(userLang) %></td>
  </tr>
  </jalios:if>
<%
