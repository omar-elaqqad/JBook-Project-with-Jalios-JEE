...
<div class="panel-heading" data-toggle="collapse" <%= Util.notEmpty(parentId) ? "data-parent=\"#" + parentId + "\"" : "" %> data-target="#<%= panelId %>">
  <% if (Util.notEmpty(getIncludeString("title", null))) { %>
    <div class='panel-actions collapse-action<%= active ? " dropup" : "" %>'>
      <jalios:icon src='<%= active ? "accordion-collapse" : "accordion-expand" %>' />
      <span class="panel-action-open"><%= glp("jcmsplugin.esn.panel-toggle-show") %></span>
      <span class="panel-action-close"><%= glp("jcmsplugin.esn.panel-toggle-hide") %></span>
    </div>
