...
!(function($) {
  ...
  // ==========================================
  //  $.jalios.smartPhone (Util)
  // ==========================================

  $.jalios.smartPhone.Util = {
    /**
     * Initializes the image uploader for this
     */
    initImageUploader: function(component) {
      $(".upload-with-image-resize", component).each(function() {
        var maxWidth = $(this).data("jalios-imageuploader-max-width")
          ? $(this).data("jalios-imageuploader-max-width")
          : $["jalios-settings"]["imageuploader-default-max-width"];
        var maxHeight = $(this).data("jalios-imageuploader-max-height")
          ? $(this).data("jalios-imageuploader-max-height")
          : $["jalios-settings"]["imageuploader-default-max-height"];
    ...
    /**
     * Gives focus to input enclosed in ".focus" in e.target.
     * If there is no ".focus", the first visible input is focused.
     * Does nothing if e.target is enclosed in a ".no-focus" element.
     */
    smartFocus: function(e) {
      if (!e) {
        return;
      }
      var $target = $(e.target);
      ...
      // bypass nofocus div
      if ($target.closest(".no-focus").length > 0) {
        return;
      }

      // Narrow search on given area
      var $focus = $target.find(".focus");
      if ($focus.length > 0) {
        $target = $focus;
      }

      var $input = $target.find(":input:visible:first");
      if ($input.closest(".no-focus").length > 0) {
        return;
      }
      if (!$input.length > 0) {
        return;
    ...
    /**
     * Enhance the response DOM, plus the enclosing ('closest') listview if any
     */
    enhanceComponent: function($component) {
      //Enhance the response DOM
      $component.enhanceWithin();
      if ($component.parent()[0]) {
        $component.parent().enhanceWithin();
      } 
      $component.filter("[data-role=popup]").popup();
      //Enhance the listview if any is found
      var $list = $component.closest(".ui-listview");
      if ($list.length > 0 && !$list.is(".ui-listview-enhanced")) {
        $list.listview("refresh");
      }
    },

    trigger: function(action, event, target, position, options) {
      var evt = jQuery.Event("jalios:refresh");
      evt.broker = {};
      ...
      evt.skip = (event && event.skip) ? event.skip : false;
