...
<jalios:cards resource="cards.jmobile-sliding" css="card-horizontal-layout-sliding">
  <jalios:foreach collection='<%= formHandler.getPageResult().getResultList() %>' type='Publication' name='itPub' skip="<%= 0 %>" max='<%= formHandler.getPortletPageSize() %>'>
    <jalios:cardData css="is-image-squared slide-card rounded-card" data="<%= itPub  %>" template="jmobile-horizontal" />
  </jalios:foreach>
</jalios:cards>